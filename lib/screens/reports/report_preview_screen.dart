...
class _ReportPreviewScreenState extends State<ReportPreviewScreen> {
              ...
              children: [
                ...
                pw.Container(
                  ...
                  child: pw.Row(
                    mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
                    children: [
                      pw.Text("AI SCAN RESULT",
                          style: pw.TextStyle(
                              fontWeight: pw.FontWeight.bold, fontSize: 14)),
                      pw.Column(children: [
                        pw.Text(
                            "${widget.finalConfidence.toStringAsFixed(0)}% Confidence",
                            style: pw.TextStyle(
                                fontSize: 18,
                                fontWeight: pw.FontWeight.bold,
        ...
        children: [
          ...
          RichText(
            text: TextSpan(children: [
              TextSpan(
                  text: "${widget.finalConfidence.toStringAsFixed(0)}",
                  style: const TextStyle(
                      fontSize: 48,
                      fontWeight: FontWeight.w900,
  ...
  // Ye function database mein final data bheje ga
  Future<void> _finalizeReport() async {
    try {
      String? uid = UserProvider.userId;

      await FirebaseFirestore.instance.collection('reports').add({
        'reportId': reportId,
        'userId': uid ?? "unknown_user",
        'userName': UserProvider.userName ?? "User",
        'statusLabel': widget.finalStatus,
        'confidence': widget.finalConfidence,
        'symptoms': widget.selectedSymptoms,
        'timestamp': FieldValue.serverTimestamp(),
        'isComplete': true,
      });

      debugPrint("✅ Phase 2: Report successfully linked to User ID: $uid");
    } catch (e) {
...
// // Ye function database mein final data bheje ga
//   // ✅ Ye function ReportPreviewScreen class ke andar hoga
//   Future<void> _finalizeReport() async {
//     try {
//       // 1. Check karein ke UserProvider se ID mil rahi hai
//       String? uid = UserProvider.userId;

//       // 2. Firestore mein report save karein
//       await FirebaseFirestore.instance.collection('reports').add({
//         'reportId': reportId,
//         'userId': uid ?? "unknown_user", // Ab ye dynamic ID uthaye ga
//         'userName': UserProvider.userName ?? "User",
//         'hbValue': widget.finalHb,
//         'symptoms': widget.selectedSymptoms,
//         'statusLabel': getStatusDetails()['label'],
//         'timestamp': FieldValue.serverTimestamp(),
//         'isComplete': true,
//       });
...
//                   // ✅ Phase 2: Final report save call
//                   await _finalizeReport();
